  ...
  return (
    <div className={`flex items-center justify-between lg:flex-col lg:justify-center nike-container ${ifExists ? 'flex-row-reverse' : 'flex-row'}`}>
      <div className='max-w-lg lg:max-w-none w-full md:text-center lg:justify-items-center'>
        ...
        <h1 className='text-5xl lg:text-4xl md:text-3xl sm:text-2xl font-bold text-slate-900 filter drop-shadow-lg ' style={{ lineHeight: '1' }}>
          {firstLine}
          <br />
          {secondLine}
        </h1>
        <p className='xl:text-sm my-4 text-slate-900'>{text}</p>
        <a href={url} className="items-center" target={"_blank"} role="button">
